...
@section('content')
    ...
    <form method="post" action="{{ route('lxd.store') }}">
        ...
        <br /><br />
        <div class="mdui-typo" style="text-align: right;margin-top: 10px"><small class="mdui-clearfix">注意：每分钟价格 = 地区服务器基础价格 + 容器模板价格 + 端口转发。<br />Linux 容器默认用户名为root，并且是无特权容器，不支持Docker。<br />带宽均为共享带宽，如果带宽有调整，将会即时生效。<br />如果你的容器配置和模板配置不符，可以对容器模板进行升配/降配，然后再修改回去即可刷新。</small></div>
    </form>
@endsection
